...
TEST(FITStable, update_columns)
{
    // Ensure the file exists from previous write tests (all_columns_single_file).
    fitsfile* rawFptr = nullptr;
    int status = 0;
    fits_open_file(&rawFptr, testurl.c_str(), READONLY, &status);
    ...
    status = 0;
    // Move to first table HDU (primary image is HDU 1, table is HDU 2).
    fits_movabs_hdu(fptr.get(), 2, &hdutype, &status);

    FITStable table(fptr, 2);

    size_t nrows = table.nrows();
    size_t pos = 0;
    EXPECT_NO_THROW(pos = table.push_back<int32_t>("COL_INT32", 25));
    EXPECT_EQ(table.nrows(), nrows + 1);
    EXPECT_EQ(pos, nrows);
    EXPECT_EQ(dynamic_cast< FITScolumn<int32_t>* >(table.getColumn("COL_INT32").get())->values<int32_t>()[nrows], 25);
    EXPECT_EQ(dynamic_cast< FITScolumn<bool>* >(table.getColumn("COL_LOG").get())->values<bool>()[nrows], false);
    EXPECT_EQ(dynamic_cast< FITScolumn<bool>* >(table.getColumn("COL_BIT").get())->values<bool>()[nrows], 0);
    EXPECT_EQ(dynamic_cast< FITScolumn<std::string>* >(table.getColumn("COL_STR").get())->values<std::string>()[nrows], "");
    EXPECT_EQ(dynamic_cast< FITScolumn<std::vector<int32_t>>* >(table.getColumn("V_INT32").get())  ->values<std::vector<int32_t>>()[nrows], std::vector<int32_t>());

    EXPECT_NO_THROW(table.updateAt<bool>("COL_LOG",nrows, true));
    EXPECT_EQ(pos, nrows);
    EXPECT_EQ(dynamic_cast< FITScolumn<bool>* >(table.getColumn("COL_LOG").get())  ->values<bool>()[nrows], true);
    EXPECT_NO_THROW(table.updateAt< std::vector<int32_t> >("V_INT32",nrows,std::vector<int32_t>({25,10,3}) ));
    EXPECT_EQ(pos, nrows);
    EXPECT_EQ(dynamic_cast< FITScolumn<std::vector<int32_t>>* >(table.getColumn("V_INT32").get())  ->values<std::vector<int32_t> >()[nrows], std::vector<int32_t>({25,10,3}));
